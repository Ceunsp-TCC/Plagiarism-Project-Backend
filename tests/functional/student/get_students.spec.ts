...
const url = '/v1/students/get-all'
const urlLogin = '/v1/auth/login'

test.group('Get students', (group) => {
  group.each.setup(async () => {
    await Database.beginGlobalTransaction()
    return () => Database.rollbackGlobalTransaction()
  })

  test('Should be found students', async ({ client }) => {
  ...
  test('Should be not found students', async ({ client }) => {
    const login = await client
      .post(urlLogin)
      .basicAuth(basicCredentials.username, basicCredentials.password)
  ...
  test('Should be filter per name student', async ({ client }) => {
    const login = await client
      .post(urlLogin)
      .basicAuth(basicCredentials.username, basicCredentials.password)
      .json(mockSchoolCredentials)
    const sut = await client
      .get(url)
      .qs({ name: 'student doest not exist' })
      .bearerToken(login.response.body.content.accessToken.token)
